...
def Single_Plot(directory,Prev_directory,ALL):
    ...
    with np.load(os.path.join(directory,"datafile.npz")) as data:
    ...
    for i in range(len(MeanList)):
        EndMean.append(MeanList[i])#np.mean(MeanList[i][-int(T/10):]))
        EndMedian.append(MedianList[i])#np.mean(MedianList[i][-int(T/10):]))
