...
for d in dirlist:
    ...
    with np.load(os.path.join(args.directory,d,filename)) as data:
        ...
        F = data['F']
        PNum = data['PNum']
        Complete_Mnum = data['Complete_Mnum']
        Complete_MeanN = data['Complete_MeanN']
        Complete_MedianN = data['Complete_MedianN']
        timetaken = data['timetaken']

    FList.append(F)
    PNumList.append(PNum)
    FinalMean.append(Complete_MeanN[-1])
...
TheoryMeanMatrix = []
for f in SortedFList:
    templist = []
    for n in SortedNList:
        templist.append(min(n/(1-f),1))
    TheoryMeanMatrix.append(templist)


fig=plt.figure(3)
ax = fig.add_subplot(1,1,1)
cp = ax.tricontourf(N.ravel(), F.ravel(), Mean.ravel(),10,cmap='coolwarm')
ax.pcolor(np.asarray(SortedNList)-dN,np.asarray(SortedFList)-dF,np.array(TheoryMeanMatrix),cmap='coolwarm')
...
plt.savefig(str(args.directory) + '/Final_Mean_Theory.png')
